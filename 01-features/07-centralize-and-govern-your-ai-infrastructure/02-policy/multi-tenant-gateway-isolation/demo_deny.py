...
def main():
    ...
    if not success:
        ...
        log.info("    MONITOR: both layers disabled. All tools visible, all calls")
        log.info("             succeed. Violations are logged to CloudWatch.")
        log.info("             Use during development to test policies safely.")
        log.info("    ENFORCE: both layers active. Production setting.")
    else:
        log.info("  WARNING: The call succeeded. This means the policy engine")
        log.info("  may be in MONITOR mode instead of ENFORCE mode.")
        log.info("  Check the Gateway configuration in the console.")
